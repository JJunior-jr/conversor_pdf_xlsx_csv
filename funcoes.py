...
def extrair_contas(pdf) -> list[dict]:
    ...
    # ─── Primeira passada: coletar todas as linhas limpas ───
    todas_linhas = []
    ...
    # ─── Segunda passada: processar com look-ahead/look-behind ───
    conta_atual = None
    esperando_ref = False
    ...
    while i < len(todas_linhas):
        ...
        if m_sd:
            ...
            # Buscar descrição na linha anterior
            desc_anterior = ""
            if i > 0:
                prev = todas_linhas[i - 1]
                # A linha anterior é a descrição se NÃO for uma linha estrutural
                if not eh_linha_estrutural(prev):
                    desc_anterior = prev
        ...
        if esperando_continuacao and conta_atual:
            # Se a próxima linha NÃO é estrutural, é continuação da descrição
            if not eh_linha_estrutural(linha):
                ...
                esperando_continuacao = False
                # Não dá continue, processa a linha normalmente abaixo
